...
class LocoCtrl(object):
    ...
    # control parameters
    cp_keys = [
        'theta_tgt', 'c0', 'cv', 'alpha_delta', # 'c0', 'cv' are used to compute the desire foot displacement 'alpha_tgt'
...
# -----------------------------------------------------------------------------------------------------------------
    def spinal_control_leg(self, s_leg, sensor_data):
        ...
        theta = s_b['theta'][1] # Pelvis angle lateral
        dtheta = s_b['dtheta'][1] # pelvis angular velocity lateral
        ...
        S_SOL_1 = ph_st*np.maximum(
            cp['SOL_1_FG']*s_l['F_SOL'] # SOL = S0 + GSOLFSOL, where FSOL is the time delayed (∆t = 20ms) muscle force signal, S0 isa small stimulation offset of 1%
            , 0)
